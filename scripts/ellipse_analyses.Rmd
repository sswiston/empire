...
data_directory_2 <- paste0(cluster_directory,"skinks/output/skinks.008/")
...
for (i in 1:nrow(ellipse_data_grouped)) {
  ...
  if (grepl("Notoscincus ornatus", taxon, fixed=TRUE)) {clade <- "Excluded"}
...
node_directions <- ggplot(spheno_shapes) +
  ...
  #geom_text(data=ellipse_data_grouped,aes(x=x,y=y,label=taxon),color=lines_color,size=2,fontface="bold",check_overlap=TRUE) +
...
Here is the tree being used, including node numbers. Three major monophyletic clades are identified: *Ctenotus*, *Lerista*, and "other genera". The "other genera" clade includes all other Australian sphenomorphine skinks, except *Notoscincus ornatus*, which is sister to the clade containing both *Ctenotus* and *Lerista*.
...
tree_plot <- ggtree(tree_2,aes(color=group),layout="dendrogram") +
  ...
  geom_cladelab(node=390,label="Other, 47 sp.",fontsize=12,barsize=1,textcolor=color_4,barcolor=color_4,offset=8,vjust=1.5,hjust=.5) +
  geom_tiplab() +
...
tree_plot_other <- ggtree(tree_other,color=color_4) +
  ...
  theme(plot.background=element_rect(fill=background_color,color=NA),
        panel.background=element_rect(fill=background_color,color=NA))
ggsave(paste0(tree_plot_directory,"tree_other.png"),tree_plot_other,dpi=600,height=20,width=10)
```

``` {r tree_plots_other_show, eval=TRUE, results=TRUE} 
include_graphics(paste0(tree_plot_directory,"tree_other.png"))
```
...
These are direction line plots for each major subclade: *Ctenotus*, *Lerista*, and "Other".
...
```{r directions_other, echo=TRUE}
node_list <- c(390:435)
directions_other <- direction_data[which(direction_data$node %in% node_list),]

# Getting true direction line counts
count_0 <- sum(directions_other$true_selection == 0)
count_1 <- sum(directions_other$true_selection == 1)
count_2 <- sum(directions_other$true_selection == 2)
count_3 <- sum(directions_other$true_selection == 3)
...
# Combining direction lines with bars
plot_with_arrows <- bar_plot + inset_element(direction_plot,-0.025,-0.05,1.025,0.45)
ggsave(paste0(direction_plot_directory,"directions_other.png"),plot_with_arrows,dpi=600)
```

```{r directions_other_show, eval=TRUE, results=TRUE}
include_graphics(paste0(direction_plot_directory,"directions_other.png"))
```
...
These are direction maps for each major subclade: *Ctenotus*, *Lerista*, and "Other".
...
```{r directions_map_other, echo=TRUE}
node_list <- c(390:435)
directions_other <- direction_data[which(direction_data$node %in% node_list),]
...
directions_other_factored <- directions_other
directions_other_factored$true_selection <- as.factor(directions_other$true_selection)
all_directions <- ggplot(spheno_shapes) +
  lims(x=c(xmin,xmax),y=c(ymin,ymax)) +
  geom_sf(fill=shape_color,color=NA) +
...
all_directions_labelled <- all_directions + inset_element(line_plot,.65,.8,.99,.99)
ggsave(paste0(direction_plot_directory,"directions_map_other.png"),all_directions_labelled,dpi=600)
```

```{r directions_map_other_show, eval=TRUE, results=TRUE}
include_graphics(paste0(direction_plot_directory,"directions_map_other.png"))
```
