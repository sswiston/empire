...
Packages required for these analyses: ape, corrr, devtools, dplyr, empire, geojsonsf, ggplot2, ggtree, HDInterval, httr2, knitr, mcmcse, patchwork, phytools, rase, raster, rgplates, sf, sfheaders, spatstat, treeio
...
Here are the precipitation rasters for each time: 40 Ma, 20 Ma, and present. These rasters are obtained from Pohl et al. 2022, and are transformed using the `rgplates` package under the PALEOMAP model.
...
# 
# remove_x <- sort(unique(raster_present_australia_data$x))[1]
# remove_y <- sort(unique(raster_present_australia_data$y),decreasing=TRUE)[1:2]
# keep <- which(!(raster_present_australia_data$x %in% remove_x & raster_present_australia_data$y %in% remove_y))
# raster_present_australia_data <- raster_present_australia_data[keep,]
# 
...
# 
# remove_x <- sort(unique(raster_20_australia_data$x))[1]
# remove_y <- sort(unique(raster_20_australia_data$y),decreasing=TRUE)[1:2]
# keep <- which(!(raster_20_australia_data$x %in% remove_x & raster_20_australia_data$y %in% remove_y))
# raster_20_australia_data <- raster_20_australia_data[keep,]
# 
...
# 
# remove_x <- sort(unique(raster_40_australia_data$x))[1]
# remove_y <- sort(unique(raster_40_australia_data$y),decreasing=TRUE)[1:2]
# keep <- which(!(raster_40_australia_data$x %in% remove_x & raster_40_australia_data$y %in% remove_y))
# raster_40_australia_data <- raster_40_australia_data[keep,]
# 
