...
data_directory <- paste0(cluster_directory,"skinks/output/test.001/")
data_directory_2 <- paste0(cluster_directory,"skinks/output/skinks.008/")
data_directory_shuffled <- paste0(cluster_directory,"skinks/output/test.003/")
...
Here are the estimates for all model parameters related to the stochastic movement of ellipses over time. I might need to rerun this analysis because it seems like $\sigma_a$ is being limited by the prior.
...
#   ymax <- max(density(data_model[,index])$y)
#   plot <- ggplot(data_model,aes_string(x=param,y="..density..")) +
...
# }
# 
# dummy_data <- data.frame(matrix(data=c(0,0,1,1,0,1,0,1),ncol=2,nrow=4))
...
#   annotate(geom="text",label="Posterior Mean",x=.5,y=.65,hjust=.5,color=lines_color) +
#   annotate(geom="text",label="95% HPD Interval",x=.5,y=.2,hjust=.5,color=lines_color) +
#   theme_bw() +
...
# ggsave(paste0(other_plot_directory,"parameters.png"),parameter_plot,dpi=600,width=8,height=4)
```
