...
Packages required for these analyses: ape, corrr, devtools, dplyr, empire, geojsonsf, ggplot2, ggtree, grid, gridExtra, HDInterval, httr2, knitr, lubridate, mcmcse, patchwork, phytools, raster, rgplates, sf, sfheaders, spatstat, stringr, treeio
...
aridity_data <- paste0(local_directory,"skinks/data/australia/")
```
...
# run_log <- read.csv(paste0(cluster_directory,"joblogs/run_log.txt"),sep="\t")
...
#   row <- which(run_data$condition == analysis_type[1] & run_data$dataset == analysis_type[2] & run_data$number == analysis_type[3])
...
data_summary <- data.frame(matrix(NA,nrow=7,ncol=5))
colnames(data_summary) <- c("Type", "Ended", "Completed", "Size", "Hours")
data_summary$Type <- c("Full Sim", "Full Sensitivity", "Full Noisy", "Full Rase", "Standard Skinks", "Shuffled Skinks", "Under Prior Skinks")
...
data_summary$Ended[7] <- sum(run_data$ended[which(run_data$condition == "under_prior" & run_data$dataset == "skinks")])
...
data_summary$Completed[7] <- sum(run_data$completed[which(run_data$condition == "under_prior" & run_data$dataset == "skinks")])
...
data_summary$Size[7] <- round(mean(run_data$tree_size[which(run_data$condition == "under_prior" & run_data$dataset == "skinks")]))
...
data_summary$Hours[7] <- round(mean(run_data$hours[which(run_data$condition == "under_prior" & run_data$dataset == "skinks" & run_data$hours > .1)]))
...
* 5 skink analyses under the prior

</div>
<div class = "col-md-6">

Here are the run times for the simulation analyses based on the sizes of the phylogenetic trees. Note that there is a logarithmic relationship. Oddly, some analyses performed much better, and seem to follow their own little trajectory.

```{r times, echo=TRUE}
run_data <- read.csv(paste0(cluster_directory,"joblogs/run_data.tsv"),sep="\t")
run_data_subset <- run_data[which(run_data$condition == "full" & run_data$dataset == "sim" & run_data$completed == TRUE),]
...
times_plot <- ggplot(run_data_subset,aes(x=tree_size,y=hours)) +
  ...
  labs(x="Tree Size (number of taxa)",y="Run Time (hours)") +
  annotate(geom="text",
           label=paste0("hours = ",round(coefs[2])," x ln(taxa) - ",round(abs(coefs[1]))),
           x=min(run_data_subset$tree_size),
           y=max(run_data_subset$hours),
           color=lines_color,
           size=6,
           hjust="inward",
           vjust="inward") +
  ...
  theme(panel.background=element_rect(fill=background_color,color=lines_color),
        ...
        panel.grid.minor.y=element_line(color=shape_color),
...
Here are the precipitation rasters for each time: 40 Ma, 20 Ma, and present. These rasters are obtained from Pohl et al. 2022, and are transformed using the `rgplates` package under the PALEOMAP model. They have been cropped to only include cells that are within Australia and that are present for all time slices. I might want to interpolate between these time slices (via per-cell linear models) to create rasters coinciding with node timings.

```{r process_rasters, echo=TRUE}
# GPLATES RECONSTRUCTIONS -- DO NOT RERUN
# sf_use_s2(FALSE)
# australia_shape <- st_union(spheno_shapes)
# australia_shape <- sf_remove_holes(australia_shape)
...
# 
# raster_20_csv <- read.csv(paste0(aridity_data,"pohl/20Ma_Pohletal2022_DIB_PhaneroContinentalClimate.csv"))
...
# 
# raster_40_csv <- read.csv(paste0(aridity_data,"pohl/40Ma_Pohletal2022_DIB_PhaneroContinentalClimate.csv"))
...
# 
# write.csv(raster_present_australia_data,paste0(aridity_data,"Ma0_raster.csv"),row.names=FALSE)
# write.csv(raster_20_australia_data,paste0(aridity_data,"Ma20_raster.csv"),row.names=FALSE)
# write.csv(raster_40_australia_data,paste0(aridity_data,"Ma40_raster.csv"),row.names=FALSE)
```
 
```{r read_rasters, eval=TRUE}
raster_present_australia_data <- data.frame(read.csv(paste0(aridity_data,"Ma0_raster.csv")))
raster_20_australia_data <- data.frame(read.csv(paste0(aridity_data,"Ma20_raster.csv")))
raster_40_australia_data <- data.frame(read.csv(paste0(aridity_data,"Ma40_raster.csv")))
```

```{r aridity_maps, echo=TRUE}
plot_raster <- function(raster_data,name) {
  plot <- ggplot(australia_shape) +
    geom_tile(data=raster_data,aes(x=x,y=y,fill=precip)) +
    scale_fill_continuous(limits=c(0,20),type="viridis") +
    labs(fill="Precipitation") +
    geom_sf(fill=NA,color=lines_color) +
    annotate(geom="text",label=name,color=lines_color,size=16,x=max(raster_data$longitude),y=max(raster_data$latitude),hjust=-.15,vjust=-.1) +
    theme_void() +
...
present_plot <- plot_raster(raster_present_australia_data,"Present")
Ma20_plot <- plot_raster(raster_20_australia_data,"20 Ma")
Ma40_plot <- plot_raster(raster_40_australia_data,"40 Ma")

aridity_plot <- Ma40_plot + Ma20_plot + present_plot + plot_layout(ncol = 3, guides = "collect") & theme(plot.background=element_rect(fill=background_color,color=NA))

ggsave(paste0(aridity_plot_directory,"aridity_maps.png"),aridity_plot,dpi=600,width=15,height=5)
```
...
Several of these *rase* analyses did not converge. Some had no variability at all, and some had very low ESS. This makes it difficult to get good data for the sigma parameters specifically. Also, some of the estimates are very poor (like 4 orders of magnitude off). I need to examine these, and see if I can improve the *rase* analyses to provide a fair comparison. So far, it looks like EMPIRE performs much better than RASE on data simulated under EMPIRE (to be expected). Interestingly, RASE not only overestimates rates (expected), but also misestimates root values substantially (less expected). Here, I define "outlier" based on the standard deviation of true values explored -- if the estimate is more than 4 standard deviations in either direction, it is an outlier.
...
Performing aridity analysis (total and clade-by-clade)
