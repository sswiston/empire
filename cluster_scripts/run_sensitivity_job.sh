...
if [ $# == 2 ]; then
	CONDITION=$2
	NUMBER=$1
	NAME="$CONDITION.sensitivity.$NUMBER"
fi
...
-a 'docker(sswiston/ellipses:2)' /bin/bash /storage1/fs1/michael.landis/Active/ellipses/cluster_scripts/run_sensitivity.sh $NUMBER $CONDITION
